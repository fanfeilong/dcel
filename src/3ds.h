...
const WORD COLOR_BYTE    = 0x0011;			// ChunkID：颜色以char存放3个分量
const WORD COLOR_FLOAT   = 0x0010;	        // ChunkID：颜色以float存放3个分量（这里的float是IEEE定义的float）;
...
typedef struct
{
	...
	vector3<double>  *pVerts;			    // 保存顶点坐标
	vector3<double>  *pNormals;			    // 保存点的法线量
	vector2f  *pTexVerts;				    // 保存纹理坐标
